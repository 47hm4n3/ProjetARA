...
public class Strategy5Init implements InitialPositionStrategy {
	...
	public Strategy5Init(String prefix) {
			...
			for (int i = 0; i < Network.size(); i++) {
				...
				if (initial_position.size() == 0) {
					...
					initial_position.put(n.getID(),
							new Position(
									CommonState.r.nextDouble() * (max_distance_x - min_distance_x) + min_distance_x,
									CommonState.r.nextDouble() * (max_distance_y - min_distance_y) + min_distance_y));
				} else {
					// choisir un voisin
					long id_neigbor = CommonState.r.nextLong(initial_position.size());
					// choisir un angle
					double angle = CommonState.r.nextDouble() * (Math.PI * 2);
					// position du voisin
					Position pos_neigbor = initial_position.get(id_neigbor);
					// choisir une distance aléatoire du voisin entre
					// distance_init_min et distance_init_max
